...
public class OnboardingController {
    ...
    public OnboardingItem updateOnboardingItem(@PathVariable Long employeeId, @PathVariable Long itemId, @RequestBody OnboardingItem updated) {
        OnboardingItem item = onboardingItemRepository.findById(itemId)
                .orElseThrow(() -> new RuntimeException("Onboarding item not found"));
        ...
        if (updated.getStatus() != null) {
            ...
            if ("done".equals(updated.getStatus())) {
                activityLogger.log(
                        ActivityLogger.initialsFrom(item.getEmployee().getName()),
                        "#dfeee5",
                        item.getEmployee().getName() + " completed: " + item.getTitle(),
                        "onboarding"
                );
            }
        }
